...
public class scr_CharacterController : MonoBehaviour
{
    ...
    [Header("Settings")]
    public PlayerSetingsModel playerSettings;
    ...
    private void Awake()
    {
        ...
        defaultInput.Weapn.Fire2Pressed.performed += e => AimingInPressed();
        defaultInput.Weapn.Fire2Released.performed += e => AimingInReleased();
    ...
    private void Update()
    {
        if (!scr_PlayerStats.isDead)
        {
            if (!CollectibleObject.isWin)
            {
    ...
    {
        newCharacterRotation.y += (isAimingIn ? playerSettings.ViewXSensivity * playerSettings.AimingSensivityEffector : playerSettings.ViewXSensivity) * (playerSettings.ViewXInverted ? -input_View.x : input_View.x) * Time.deltaTime;
        transform.localRotation = Quaternion.Euler(newCharacterRotation);

        newCameraRotation.x += (isAimingIn ? playerSettings.ViewYSensivity * playerSettings.AimingSensivityEffector : playerSettings.ViewYSensivity) *  (playerSettings.ViewYInverted ? input_View.y : -input_View.y) * Time.deltaTime;
    ...
    private void CalculateMovement()
    {
        ...
        if (isSprinting && currentStamina > 0 && canSprint)
        {
        ...
        else if (currentStamina <= 0 || !canSprint)
        {
            isSprinting = false;
        }
        ...
        else if (isAimingIn && !isSprinting)
        {
    ...
    {
        if (input_Movement.y <= 0.2f || !canSprint || playerStance == PlayerStance.Crouch || playerStance == PlayerStance.Prone)
        {
            isSprinting = false;
            return;
        }
